...
from items import EwgScraperIngredient, EwgScraperProduct
...
class EwgSkindeepSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for tr in response.xpath(self.product_tr_list_xpath):
            ...
            if uri and uri not in self.crawledProductUrls:
                ...
                product_url = urljoin(response.url, uri)
                ...
                product_request.meta['product_id'] = urlsafe_b64encode(data_bytes)
    ...
    def parse_product(self, response):
        ...
        for uri in response.xpath(self.ingredient_link_xpath).extract():
            ingredient_url = urljoin(response.url, uri)
            data_bytes = uri.encode("utf-8")
            ingredient_id = urlsafe_b64encode(data_bytes)
